/*
	[10158 문제] 개미
	문제 - 가로 길이가 w이고 세로 길이가 h인 2차원 격자 공간이 있다. 이 격자는 아래 그림처럼 왼쪽 아래가 (0,0)이고 오른쪽 위가 (w,h)이다. 
	...
	입력 - 첫줄에는 w와 h가 공백을 사이에 두고 주어진다. 그 다음 줄에는 초기 위치의 좌표값 p와 q가 공백을 사이에 두고 주어진다. 3번째 줄에는 개미가 움직일 시간 t가 주어진다. 
	출력 - 출력은 t 시간 후에 개미의 위치 좌표 (x,y)의 값 x와 y를 공백을 사이에 두고 출력한다. 
	예제 입력  - 6 4
			 4 1
			 8
	예제 출력  - 0 1
...
public class Num10158 {
	...
	public static void main(String[] args){
		...
		for(int i = 0 ; i < time ; i ++) {
			startW += addW;
			startH += addH;
			if(startW == w || startW ==0) addW *= (-1); 
			if(startH == h || startH ==0) addH *= (-1);				
		}
